    ...
    <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
      <div className="relative h-64 ">
        <img
          ...
          alt={hero.name}
          className={cn(
            "object-cover transition-all duration-500 group-hover:scale-110 absolute top-[-30px] w-full h-[410px] cursor-pointer",
          )}
        />
        ...
        >
          <Heart
            className={cn(
              "h-4 w-4",
              isFavorite(hero) ? "fill-red-500" : "fill-white",
            )}
          />
        </Button>
      ...
      <CardHeader className="py-3 z-10 bg-gray-100/50 backdrop-blur-sm relative top-1 group-hover:top-[-10px] transition-all duration-300">
        <div className="flex justify-between items-start">
          ...
          <Badge
            className={cn(
              "text-xs",
              hero.category === "Hero" ? "bg-blue-800" : "bg-red-800",
            )}
          >
